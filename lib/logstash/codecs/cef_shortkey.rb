...
class LogStash::Codecs::CEF < LogStash::Codecs::Base
  ...
  # # short_key
  # # deviceReceiptTime => 
  # # true: use short key e.g. rt 
  # # false: do not use short key e.g. deviceReceiptTime
  # config :short_key, :validate => :boolean, :default => false
  ...
  def handle(data, &block)
    ...
    if not message.nil? and message.include? '='
      ...
      # end
      message = message.map {|s| k, v = s.split('***'); "#{k}=#{v}"}
